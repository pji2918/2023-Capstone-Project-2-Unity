    ...
    public class AifyEditor : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            if (engine == Engine.aiFyXL_0_9)
            {
                ...
                img_dim.x = int.Parse(XL0_9_options[dim_index.x]);
                img_dim.y = int.Parse(XL0_9_options[dim_index.y]);
            }
            else
            {
                EditorGUILayout.LabelField("Width", GUILayout.MaxWidth(40));
            ...
            // CONCEPT ART BROWSER TEMPORARILY DOWN 
            // aGUILayout.BeginVertical("window");
            ...
            if (GUILayout.Button("Upscale Image", GUILayout.Height(30)))
            {
                ...
                else
                {
                    ...
                    this.StartCoroutine(PostUpscale(System.Text.Encoding.UTF8.GetString(Convert.FromBase64String("aHR0cDovLzM0LjIyNS4xNTkuOTo1MDAwL2RhdGE=")), "{ \"engine\":\"" + "upscale" + "\",\"data\":\"" + "" + "\",\"neg\":\"" + "" + "\",\"width\":\"" + "0" + "\",\"height\":\"" + "0" + "\",\"steps\":\"" + "0" + "\",\"cfgScale\":\"" + "0" + "\",\"sampler\":\"" + "" + "\",\"seed\":\"" + "1" + "\",\"initImg\":\"" + $"{base64encJpg}" + "\",\"img2imgStrength\":\"" + "1" + "\",\"mask\":\"" + "" + "\"}"));
                    initImporter.sRGBTexture = true;
                    initImporter.SaveAndReimport();
            ...
            EditorGUILayout.Space();
            // DEPTH MAP TEMPORARILY DISABLED
        ...
        IEnumerator Post(string url, string bodyJsonString)
        {
            ...
            {
                Debug.Log("Error Type: " + request.error);
                if(request.responseCode == 500)
                {
                    Debug.Log("Error in generation: This error may be caused either due to the server being temporarily down or a prompt that may contain NSFW or offensive language. For more information, please read the documentation on error types and troubleshooting.");
                }
                else if(request.responseCode == 400)
                {
                    Debug.Log("Error in generation: This error may be caused due to bad formatting of the prompt or the usage of illegal characters like quotation marks and line breaks. Please make sure you use only letters and commas in the prompt section.");
                }
            }
            else
            {
